...
<template>
  <div
    class="group flex items-center gap-4 px-5 py-4 border-b border-gray-100/80 last:border-b-0 hover:bg-gradient-to-r hover:from-primary/5 hover:to-secondary/5 transition-all duration-200"
    :class="{ 'bg-green-50/50': todo.completed }"
  >
    <button
    ...
    <template v-if="isEditing">
      <input
        ...
        class="flex-1 px-3 py-2 text-base border-2 border-primary rounded-xl focus:outline-none focus:ring-4 focus:ring-primary/10 transition-all duration-200"
        v-focus
      />
      <button
      ...
      <button
        class="px-3 py-2 text-sm font-medium text-gray-500 bg-gray-100 hover:bg-gray-200 rounded-xl transition-all duration-200 cursor-pointer"
        @click="cancelEdit"
      >
        <X class="w-4 h-4" />
      </button>
    ...
    <template v-else>
      ...
      <div class="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
        <button
          class="p-2 text-gray-400 hover:text-primary hover:bg-primary/10 rounded-xl transition-all duration-200 cursor-pointer"
          @click="startEdit"
        >
          <Pencil class="w-4 h-4" />
        </button>
        <button
          class="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-xl transition-all duration-200 cursor-pointer"
