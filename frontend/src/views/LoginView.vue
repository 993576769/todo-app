...
<template>
  <div class="min-h-screen flex items-center justify-center p-4">
    <div class="w-full max-w-[420px] bg-white/80 backdrop-blur-xl rounded-2xl card-shadow-lg p-8 transition-all duration-300 hover:shadow-xl">
      <!-- Logo & Title -->
      <div class="text-center mb-8">
        ...
        <p class="text-gray-500 mt-2">{{ isLogin ? '登录你的账户' : '创建新账户' }}</p>
      </div>

      <form @submit.prevent="handleSubmit" class="flex flex-col gap-5">
        <div v-if="!isLogin" class="flex flex-col gap-2">
          <label class="text-sm font-medium text-gray-700">姓名（可选）</label>
          <div class="relative group">
            <User class="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400 group-focus-within:text-primary transition-colors" />
            <input
              v-model="name"
              type="text"
              class="w-full pl-12 pr-4 py-3.5 text-base bg-gray-50/50 border-2 border-gray-200 rounded-xl focus:outline-none focus:border-primary focus:bg-white focus:ring-4 focus:ring-primary/10 transition-all duration-200"
              placeholder="你的名字"
            />
          </div>
        </div>
        ...
        <div v-if="error" class="bg-red-50 border border-red-100 text-red-600 p-3.5 rounded-xl text-sm text-center flex items-center justify-center gap-2">
